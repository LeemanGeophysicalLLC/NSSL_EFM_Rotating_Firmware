...
// Decimation FIFO (no SPI in ISR)
volatile uint8_t hfIdx = 0;          // 0..49 within the current PPS second
volatile uint8_t sub5  = 0;          // counts 0..4 DRDY edges
volatile uint8_t qHead = 0, qTail = 0;
volatile uint8_t idxQ[64];           // ring buffer of decimated indices
...
static inline void writeHF(File &f, const HFRecord &r, char* line, size_t cap) {
  // Scale IMU:
  // accX() is in g    -> mg (×1000)
  // gyrX() is in dps  -> mdps (×1000)
  // magX() is in µT   -> 0.1µT (×10) to fit nicely in int16
  int16_t ax = (int16_t)lroundf(r.imu[0] * 1000.0f);
  int16_t ay = (int16_t)lroundf(r.imu[1] * 1000.0f);
  int16_t az = (int16_t)lroundf(r.imu[2] * 1000.0f);
  //int16_t gx = (int16_t)lroundf(r.imu[3] * 1000.0f);
  //int16_t gy = (int16_t)lroundf(r.imu[4] * 1000.0f);
  //int16_t gz = (int16_t)lroundf(r.imu[5] * 1000.0f);
  int16_t mx = (int16_t)lroundf(r.imu[6] * 1.0f);
  int16_t my = (int16_t)lroundf(r.imu[7] * 1.0f);
  int16_t mz = (int16_t)lroundf(r.imu[8] * 1.0f);

  // Preformat once, then single write()
  int n = snprintf(line, cap,
    "1,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%d,%d,%d,%d,%d\n",
    (unsigned long)r.pps_utc,
    (unsigned long)r.ms_offset,
    (unsigned long)r.processor_ms,
    (unsigned long)r.adc[0],
    (unsigned long)r.adc[1],
    (unsigned long)r.adc[2],
    (unsigned long)r.adc[3],
    ax, ay, az,           // keep accel
    mx, my, mz            // keep mag
  );
  f.write(line, n);
...
static inline void writeLF(File &f, const LFRecord &l, char* line, size_t cap) {
  int n = snprintf(line, cap,
    "2,%lu,%lu,%lu,%ld,%ld,%u,%d,%d,%d\n",
    (unsigned long)l.pps_utc,
    (unsigned long)l.ms_offset,
    (unsigned long)l.processor_ms,
    (long)l.lat,
    (long)l.lon,
    (unsigned)l.alt,
    (int)l.temp,
...
void ppsISR() {
  ...
  hfIdx = 0;            // next decimated sample will be 0 in the new second
}
...
// ------------------------------------------------------
void adcISR() {
  // Count raw DRDY edges (250 Hz)
  if (++sub5 >= 5) {
    sub5 = 0;
    uint8_t nHead = (qHead + 1) & 63;
    if (nHead != qTail) {            // drop if full
      idxQ[qHead] = hfIdx;           // which 20 ms slot this sample belongs to
      qHead = nHead;
    }
    if (++hfIdx >= 50) hfIdx = 0;    // 50 samples per second at 20 ms each
  }
}
...
void rolloverFileIfNeeded() {
  ...
  if (logFile) {
    logFile.preAllocate(4UL * 1024UL * 1024UL); // ~4 MB chunk
  }
...
// ------------------------------------------------------
void flushLog(size_t budget = 32) {
  static uint32_t lastFlushMs = 0;
  if (!logFile) return;
  if (logBuf.size() < 32) return;

  LogEntry entry;
  char line[256]; // big enough for HF

  for (size_t i = 0; i < budget; ++i) {
    if (!logBuf.pop(entry)) break;

    if (entry.type == REC_HF) {
      writeHF(logFile, entry.data.hf, line, sizeof(line));
    } else {
      writeLF(logFile, entry.data.lf, line, sizeof(line));
    }

    // Keep draining GPS during long bursts
    if ((i & 7) == 7) pollGPS();
  }

  // Flush at ~10 Hz (or if we’re about to roll over)
  uint32_t now = millis();
  if (now - lastFlushMs >= 100) {
    logFile.flush();
    lastFlushMs = now;
  }
}
...
void loop() {
  ...
  // Process at most N HF records per loop tick to avoid stalls
uint8_t hfBudget = 8;
while (hfBudget-- && (qTail != qHead)) {
  uint8_t idx;
  noInterrupts();
  idx = idxQ[qTail];
  qTail = (qTail + 1) & 63;
  ...
  rec.ms_offset    = (uint32_t)idx * 20;              // *** exact 20 ms grid ***
  rec.processor_ms = millis();
  rec.adc[0] = tmp.ch0; rec.adc[1] = tmp.ch1; rec.adc[2] = tmp.ch2; rec.adc[3] = tmp.ch3;
